...
def test_load_settings_defaults_to_tidal_home_paths(tmp_path, monkeypatch) -> None:
    ...
    assert settings.resolved_pricing_policy_path == app_home / "auction_pricing_policy.yaml"
...
def test_load_settings_uses_explicit_env_and_pricing_policy_overrides(tmp_path, monkeypatch) -> None:
    home_root = tmp_path / "home"
    app_home = home_root / ".tidal"
    app_home.mkdir(parents=True)
    (app_home / "config.yaml").write_text("db_path: state/home.db\n", encoding="utf-8")
    ...
    monkeypatch.setenv("TIDAL_PRICING_POLICY_PATH", str(explicit_policy_path))

    settings = load_settings()

    assert settings.resolved_env_path == explicit_env_path
    assert settings.resolved_pricing_policy_path == explicit_policy_path
